...
Dlatego w niniejszej pracy zdecydowano się na zoptymalizowaną, odwróconą architekturę systemową. Cała elektronika obliczeniowa i wykonawcza została zintegrowana bezpośrednio wewnątrz ruchomej, hemisferycznej głowicy. Taki zabieg zredukował do absolutnego minimum liczbę przewodów pracujących na zginanie i skręcanie – do wnętrza kopuły wprowadzono jedynie niezbędne okablowanie od zewnętrznie zamontowanych silników krokowych. Taka topologia, w odróżnieniu od napędów współosiowych wbudowanych w jarzmo, pozwala na ułatwione odprowadzanie ciepła z korpusów silników, zapewnia łatwy dostęp serwisowy oraz umożliwia bezinwazyjną wymianę przełożeń zębatych w fazie prototypowania.

Aby zapewnić stabilność tak obciążonej konstrukcji głowicy i zapobiec jej chwianiu podczas nagłych nawrotów, odstąpiono od stosowania cienkich osi obrotu. Zamiast tego zaimplementowano wielkogabarytowe łożysko wieńcowe o średnicy zewnętrznej $140\text{ mm}$ i wewnętrznej $80\text{ mm}$. Jego zastosowanie pozwoliło na maksymalne poszerzenie bazy podparcia dla elementów obrotowych, co skutecznie przenosi momenty wywracające i niweluje luzy promieniowe układu.
...
Wszystkie elementy mechaniczne demonstratora zostały zamodelowane z wykorzystaniem środowiska projektowego CAD 3D. Cyfrowe odwzorowanie konstrukcji pozwoliło na wczesną detekcję kolizji, odpowiednie tolerowanie otworów pod wały napędowe oraz weryfikację całkowitej przestrzeni roboczej robota.
...
\subsection{Topologia zasilania i zarządzanie energią}

Kluczowym wyzwaniem w projektowaniu układów mechatronicznych jest odizolowanie elektroniki logicznej od zakłóceń wprowadzanych przez obciążenia indukcyjne. Silniki krokowe podczas pracy generują przepięcia oraz chwilowe spadki napięcia, które mogą powodować niestabilność zasilania i restarty mikrokomputera. Aby temu zapobiec, zaimplementowano gwiaździstą topologię dystrybucji energii z fizyczną separacją obwodów na bazie płytek stykowych.

Źródło zasilania stanowi pakiet czterech ogniw litowo-jonowych (Li-Ion) połączonych szeregowo. Bezpośrednio na dodatnim biegunie wyjściowym koszyka z ogniwami zainstalowano bistabilny główny wyłącznik zasilania. Jego lokalizacja na początku obwodu umożliwia awaryjne odcięcie prądu z całego urządzenia w przypadku niepożądanego zachowania mechaniki. Z głównego węzła zasilającego wyprowadzono trzy niezależne gałęzie:

\begin{enumerate}
    \item \textbf{Gałąź wysokoprądowa (wykonawcza):} Napięcie z pakietu ogniw poprowadzono bezpośrednio do złącz zasilających (VMOT) obu sterowników silników krokowych Pololu 36v4. Sterowniki te posiadają własne układy regulacji prądu, co pozwala na zasilanie cewek wyższym napięciem i zapewnia silnikom odpowiednią dynamikę przy zachowaniu bezpiecznego natężenia.
    \item \textbf{Gałąź logiczna (Raspberry Pi):} Pokładowa jednostka obliczeniowa wymaga stabilizowanego napięcia $5\text{ V}$ o dużej wydajności prądowej. W tym celu wykorzystano przetwornicę obniżającą napięcie \textbf{Pololu D24V50F5 (nr kat. 2851)}, gwarantującą wydajność rzędu $5\text{ A}$. Zapobiega to zjawisku spadku napięcia na magistrali zasilającej Raspberry Pi w momentach intensywnego obciążenia procesora.
    \item \textbf{Gałąź peryferyjna (Laser):} Prowadzona przez odrębną przetwornicę step-down o wydajności $3\text{ A}$ (napięcie wyjściowe $5\text{ V}$). Oddzielenie zasilania lasera od logiki głównej stanowi dodatkowe zabezpieczenie stabilności systemu. Mikrokontroler ESP32 zasilany jest natomiast kaskadowo, bezpośrednio z magistrali USB mikrokomputera Raspberry Pi.
\end{enumerate}

\subsection{Schemat połączeń i sprzętowe interfejsy komunikacyjne}

Architektura komunikacyjna między systemem nadrzędnym (Raspberry Pi) a układem wykonawczym (ESP32) opiera się na interfejsie szeregowym realizowanym za pomocą przewodu micro-USB. Takie połączenie zapewnia dużą odporność na zakłócenia wewnątrz obudowy oraz pozwala na przesyłanie wyliczonych parametrów sterujących z minimalnym opóźnieniem transportowym, wykorzystując prędkość transmisji na poziomie $115200\text{ bps}$.

Wysterowanie silników krokowych zrealizowano przy pomocy jednokierunkowych sygnałów logicznych \texttt{STEP} (krok) oraz \texttt{DIR} (kierunek). Mikrokontroler ESP32, operujący w standardzie napięciowym $3.3\text{ V}$, łączy się bezpośrednio z wejściami sterującymi układów Pololu. Z uwagi na wymagania dotyczące momentu obrotowego, zoptymalizowano sposób podłączenia cewek silników NEMA 17. Choć zastosowane silniki posiadają 6 wyprowadzeń i są fabrycznie przystosowane do pracy w układzie unipolarnym, połączono je w konfiguracji \textbf{bipolarnej szeregowej}, wykorzystując wyłącznie 4 przewody. Dwa środkowe odczepy cewek (tzw. \textit{center taps}) pozostawiono niepodłączone. Taka topologia zmusza prąd do przepływu przez całą długość uzwojenia, co znacząco zwiększa dostępny moment trzymający oraz moment przy niskich prędkościach obrotowych – parametr istotny dla napędu osi PAN.

Sterowanie emiterem laserowym wymagało zaimplementowania układu dopasowującego, ponieważ cyfrowe piny GPIO w ESP32 nie dysponują odpowiednią wydajnością prądową, a ich napięcie logiczne wynosi $3.3\text{ V}$. Aby rozwiązać ten problem, zaimplementowano układ kluczujący zrealizowany na tranzystorze bipolarnym \textbf{NPN 2N2222A}. Sygnał logiczny z mikrokontrolera doprowadzono do bazy tranzystora poprzez rezystor ograniczający, natomiast obwód zasilania lasera zintegrowano z obwodem kolektora. Rozwiązanie to gwarantuje separację sygnału sterującego od prądu zasilającego emiter i umożliwia sprzętowe uruchamianie wiązki celowniczej.
...
\section{Synteza regulatora PID}
Utrzymanie plamki lasera na poruszającym się obiekcie zrealizowano początkowo przy użyciu algorytmu opartego na klasycznym regulatorze PID. Przestrzeń uchybu zdefiniowano w dziedzinie pikseli, odległość środka ciężkości wykrytego obiektu od geometrycznego centrum matrycy kamery stanowi błąd wejściowy. Z uwagi na charakterystykę obiektu sterowania (silniki krokowe trzymające pozycję wprowadzają naturalne całkowanie w układzie), zrezygnowano z członu całkującego, implementując w praktyce regulator PD. Sygnał sterujący $u(t)$ opisany jest wzorem:
$$u(t) = K_p \cdot e(t) + K_d \cdot \frac{de(t)}{dt}$$

Aby zweryfikować optymalność heurystycznie dobranych nastaw, w ramach badań przeprowadzono syntezę z wykorzystaniem pierwszej metody Zieglera-Nicholsa (metody odpowiedzi skokowej w pętli zamkniętej). Procedura ta polegała na wyzerowaniu wzmocnienia różniczkującego ($K_d = 0$) i stopniowym zwiększaniu wzmocnienia proporcjonalnego $K_p$, aż do momentu wprowadzenia platformy w stan niegasnących oscylacji. Zanotowana wartość wzmocnienia krytycznego $K_{kr}$ oraz odczytany z logów systemowych okres oscylacji $T_{kr}$ posłużyły do wyznaczenia docelowych parametrów regulatora. Na podstawie tabel Zieglera-Nicholsa dla regulatora PD nastawy te wyrażają się empirycznymi relacjami $K_p = 0.8 \cdot K_{kr}$ oraz $K_d = K_p \cdot 0.125 \cdot T_{kr}$.

\section{Synteza regulatora trójpołożeniowego}
W celach komparatystycznych w systemie zaimplementowano alternatywny, nieliniowy algorytm sterowania oparty na regulatorze trójpołożeniowym (typu Bang-Bang). W przeciwieństwie do regulatora ciągłego, wymusza on stałą prędkość ruchu efektora skierowaną w stronę uchybu, przyjmując jedynie trzy stany wyjściowe. Kluczowym parametrem tego układu jest szerokość strefy nieczułości (ang. deadband) zdefiniowana wokół punktu docelowego. 

Synteza tego regulatora stanowiła kompromis pomiędzy dokładnością pozycjonowania a stabilnością energetyczną układu. Zbyt wąska strefa nieczułości wywoływała zjawisko chatteringu, charakteryzujące się szybkimi oscylacjami wokół zera, co prowadziło do drastycznych zapadów napięcia wskutek ciągłych rewersji silników. Dobór optymalnej strefy nieczułości oraz maksymalnej prędkości korekcyjnej został wyznaczony doświadczalnie, gwarantując zatrzymanie mechaniki przed wyjściem obiektu poza tolerowany obszar ramki z jednoczesnym uwzględnieniem opóźnień transportowych wprowadzanych przez analizę wizyjną. Wykresy odpowiedzi skokowej dla obu wariantów sterowania posłużą do rzetelnej oceny wydajności zastosowanych algorytmów.
...
\subsection{stanowisko badawcze}
%\begin{itemize}
...
%
%
\section{Strojenie i analiza działania regulatora PID}
\section{Porównanie skuteczności systemów}
%
%\begin{itemize}
