...
\begin{enumerate}
    \item \textbf{Płynnego regulatora proporcjonalno-różniczkującego (PD/PID)}, którego nastawy poddawane są początkowej optymalizacji w dedykowanym środowisku symulacyjnym Gazebo (\textit{auto-tuning}).
...
\end{equation}
gdzie $\mu$ oznacza zaimplementowany mnożnik podziału mikrokroku. Odpowiednie zwiększenie przełożenia redukcyjnego było niezbędne do utworzenia sprzętowego filtru dolnoprzepustowego, zdolnego do wygładzenia mikroskoków silnika i redukcji hałasu mechanicznego wywoływanego przez drobne oscylacje algorytmu śledzącego punkt docelowy. Zintegrowanie opisanego układu przeniesienia napędu z pozostałymi elementami konstrukcyjnymi pozwoliło na finalny montaż platformy. Gotowy, fizyczny model demonstratora zaprezentowano na Rysunku \ref{fig:gotowy_model}.
...
Integralną częścią całego środowiska symulacyjnego jest zamknięty wirtualny świat, w którym wieżyczka przeprowadza proces śledzenia (Rys. \ref{fig:gazebo_sim}). Środowisko to zostało zdefiniowane w pliku \texttt{target\_world.sdf} i korzysta z silnika fizyki ODE (ang. Open Dynamics Engine). Oprócz podstawowych uwarunkowań takich jak grawitacja, oświetlenie kierunkowe czy płaszczyzna podłoża, wykreowano w nim dynamiczny obiekt referencyjny, nazywany kulką treningową. Obiekt ten powołano jako encję typu \texttt{actor}, co pozwala na jego przemieszczanie się w sposób deterministyczny, z pominięciem obciążeń wynikających z przeliczania rozbudowanej fizyki zderzeń. 
...
Bez implementacji mechanizmów adaptacyjnych (rys. \ref{fig:pid_oddalanie}a), układ operujący na stałych nastawach tracił stabilność w pierwszej fazie testu. Ponieważ z odległości 30~cm obiekt zajmował niemal ćwierć kadru i generował ogromne przyrosty uchybu przy najmniejszym ruchu, wieżyczka wpadała w gwałtowne oscylacje, które wygasły dopiero w połowie testu, gdy obiekt oddalił się na wystarczający dystans. Taka charakterystyka dyskwalifikuje klasyczny układ regulacji w zastosowaniach obronnych, gdzie cel może gwałtownie zbliżyć się do stanowiska.
...
\subsection{Test działania algorytmu w warunkach środowiskowych (Outdoor)}
...
\begin{figure}[H]
    ...
    \caption{Kadr ze strumienia wideo podczas testów polowych. Widoczny dron przenoszący obiekt testowy na tle infrastruktury i roślinności, oddalony o około 15 metrów od obiektywu.}
    \label{fig:terrain_kadr}
\end{figure}

Wykres odpowiedzi układu w tych rygorystycznych warunkach (Rys. \ref{fig:terrain_wykres}) jest bezpośrednim dowodem na wysoką krzepkość (\textit{robustness}) zaprojektowanego systemu sterowania. W początkowej i końcowej fazie wykresu widać, że wieżyczka niezwykle skutecznie sprowadzała uchyb do bezpiecznej strefy tolerancji, płynnie podążając za szybko manewrującym statkiem powietrznym.
...
Szczególnej uwagi wymaga środkowy fragment przebiegu, na którym widoczne są gwałtowne, prostokątne piki odchylenia. Stanowią one zapis momentu, w którym dron oddalił się poza roboczy zasięg efektywnej detekcji. Prosty algorytm klasyfikacji przestrzeni barw HSV zawiódł, a system wizyjny "złapał" fałszywy cel w postaci jaskrawozielonych liści drzew w tle, których reprezentacja wektorowa w modelu HSV bywa zbliżona do wyblakłego odcienia żółci w ostrym słońcu. Wieżyczka natychmiastowo zareagowała na ten bodziec, co ilustruje nagły skok uchybu na wykresie. Najważniejszym osiągnięciem systemu jest jednak fakt re-akwizycji celu. Gdy tylko dron powrócił w kadr i algorytm wizyjny ponownie wyłuskał prawidłowy kontur, wieżyczka bez wahania porzuciła szum z tła i gładko zniwelowała potężny uchyb, powracając do bezbłędnego śledzenia wewnątrz strefy nieczułości (widoczne w prawej części wykresu). 
...
Budowa fizycznego demonstratora obnażyła szereg trudności mechanicznych i sprzętowych, które muszą zostać uwzględnione przy projektowaniu systemów docelowych pracujących w warunkach polowych (\textit{outdoor}).

Głównym problemem okazał się dobór układu nadrzędnego. Zastosowanie Raspberry Pi, choć budżetowe i wygodne w fazie prototypowania, w ogólnym rozrachunku okazało się słabym wyborem dla mobilnego systemu obronnego. Komputer ten cechuje się ogromną "wybrednością" zasilania -- przy próbie jednoczesnego przetwarzania wizji, sterowania napędami i uruchomienia strumieniowania wideo przez sieć, spadki napięcia na zasilaniu bateryjnym powodowały nieoczekiwane restarty jednostki. Pełną stabilność i wysoką częstotliwość analizy obrazu udawało się utrzymać jedynie przy wykorzystaniu wydajnych zasilaczy sieciowych, co drastycznie ogranicza mobilność platformy. 

Dodatkowo, obecna konstrukcja wieżyczki ma charakter wyłącznie demonstracyjny (otwarty). Praca w warunkach terenowych kategorycznie wymaga szczelnej zabudowy elektroniki i zabezpieczenia elementów ruchomych przed pyłem i wilgocią.

Na podstawie przeprowadzonych prac sformułowano następujące rekomendacje konstrukcyjne:
\begin{itemize}
    \item Systemy wizyjne operujące w zmiennym oświetleniu nie mogą w pełni polegać na algorytmach bazujących wyłącznie na kontraście, odejmowaniu tła i przestrzeni barw (HSV), gdyż prowadzi to do fałszywych detekcji (np. jaskrawe liście w słońcu).
    \item Konieczna jest stabilizacja zasilania obwodów wizyjnych na wysoce wydajnych przetwornicach typu \textit{Buck-Boost}, niezależnych od zasilania serwomechanizmów.
    \item Przy niewielkim budżecie rzędu 1500 PLN udało się udowodnić koncepcję (\textit{Proof of Concept}). Zwiększenie budżetu do poziomu zaledwie 3000 -- 4000 PLN pozwoliłoby na wdrożenie układów o gigantycznie wyższej efektywności, całkowicie eliminując zaobserwowane wąskie gardła.
\end{itemize}
